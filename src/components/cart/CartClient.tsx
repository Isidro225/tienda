...
export function CartClient() {
    ...
    // ✅ ya no pedimos teléfono, eso lo toma el server del usuario verificado
    const canSubmit = items.length > 0 && customerName.trim().length >= 2;
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="rounded-2xl border border-app-border bg-white p-5 shadow-card">
                ...
                <p className="mt-1 text-sm text-app-muted">
                    Para realizar pedidos necesitás estar registrado y verificado por WhatsApp.
                </p>
                    ...
                    <Button
                        ...
                        onClick={() =>
                            start(async () => {
                                ...
                                const msg = res.error ?? "No se pudo crear el pedido.";

                                // ✅ si no está logueado o no está verificado, mandamos a login
                                if (msg.includes("iniciar sesión") || msg.includes("verificar tu teléfono")) {
                                    router.push("/login?next=/pedido");
                                    return;
                                }
